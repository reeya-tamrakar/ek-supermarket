...
class Home extends Component {
    componentDidMount() {
        // <!-- main slider-banner -->
		jQuery('#demo1').skdslider({'delay':5000, 'animationSpeed': 2000, 'showNav': false});
	...
	HomeFn(){
		...
		let newOffersProducts = products.slice(0,4)
			...
			<div className="newproducts-w3agile">
				<div className="container">
					<h3>New offers</h3>
						<div className="agile_top_brands_grids">
							{
								newOffersProducts.map(product => <ProductItem size="col-md-3" title={product.title} op={product.unitPrice[0].oldPrice} np={product.unitPrice[0].newPrice} url={product.images[0].imageName} hasOffer={true}  />)
							}
							<div className="clearfix"> </div>
						</div>
				</div>
